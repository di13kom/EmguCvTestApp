...
{
    class IngamePlayersTestSVM
    {
        SVM p1SvmModel;
        SVM p2SvmModel;


        public IngamePlayersTestSVM(string pathPlayer1SvmModel, string pathPlayer2SvmModel)
        {
            p1SvmModel = new SVM();
            p2SvmModel = new SVM();
    ...
    class IngamePlayersTestKN
    {
        KNearest p1KnModel;
        KNearest p2KnModel;


        public IngamePlayersTestKN(string pathPlayer1SvmModel, string pathPlayer2SvmModel)
        {
            p1KnModel = new KNearest();
            p1KnModel.DefaultK = 3;
            p2KnModel = new KNearest();
            p2KnModel.DefaultK = 3;

            //player1 model load from file
            FileStorage fs1 = new FileStorage(pathPlayer1SvmModel, FileStorage.Mode.Read);

            p1KnModel.Read(fs1.GetRoot());
            fs1.ReleaseAndGetString();
            //
        ...
        public float[] PredictImage(Image<Gray, byte> p1Img, Image<Gray, byte> p2Img)
        {
            ...
            try
            {
                ...
                var x = p1KnModel.Predict(p1TestMatrix, p1PredictionMatrix,300);
                var y = p2KnModel.Predict(p2TestMatrix, p2PredictionMatrix,300);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
